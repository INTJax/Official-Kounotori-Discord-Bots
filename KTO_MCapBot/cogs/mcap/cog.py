...
class MCap(commands.Cog):
    """Keeps track of and updates it's nickname to the KTO market cap (USD)"""

    #Configure cog logging
    global logger
    logger = logging.getLogger("discord.bot.mcap")

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        logger.info("mcap cog initialised")
    
    #Set variables required for mcap updates
    global OldAmount
    OldAmount = "0"
    global DataSource
    ...
    async def getKTOData():
        ...
        try:
            ...
            logger.error(f"There was an error reading mcap data! Error was: {e}")
        ...
        for server_id in serverconfig.configuration:
            if serverconfig.configuration[server_id] != None:
                ...
                if CurrentNick != Amount:
                    ...
                    await MCap.updateOneNickname(ctx, Amount, DiscGuild)

    #Main function that controls the frequency of retrieving data and updating nicknames
    @tasks.loop(seconds=10)
    async def updateprice(ctx):
        logger.info("Checking for new mcap...")
        global OldAmount
        Amount = await MCap.getKTOData()
        if Amount != OldAmount:
            logger.info("Amount has changed to %s", f"${Amount}")
            await MCap.updateNickname(ctx, f"${Amount}")
            OldAmount = Amount
        else:
            logger.info("Amount has not changed")
            await MCap.checkNickname(ctx, f"${Amount}")

def setup(bot: commands.Bot):
    bot.add_cog(MCap(bot))
