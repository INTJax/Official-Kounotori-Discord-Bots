...
class Holders(commands.Cog):
    """Keeps track of and updates it's nickname to the KTO holder count"""

    #Configure cog logging
    global logger
    logger = logging.getLogger("discord.bot.holders")

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        logger.info("holders cog initialised")
    
    #Set variables required for holders updates
    global OldAmount
    OldAmount = "0"
    global DataSource
    ...
    async def getKTOData():
        ...
        try:
            ...
            logger.error(f"There was an error reading holders data! Error was: {e}")
        ...
        for server_id in serverconfig.configuration:
            if serverconfig.configuration[server_id] != None:
                ...
                if CurrentNick != Amount:
                    ...
                    await Holders.updateOneNickname(ctx, Amount, DiscGuild)

    #Main function that controls the frequency of retrieving data and updating nicknames
    @tasks.loop(seconds=10)
    async def updatevalue(ctx):
        logger.info("Checking for new holders...")
        global OldAmount
        Amount = await Holders.getKTOData()
        if Amount != OldAmount:
            logger.info("Amount has changed to %s", Amount)
            await Holders.updateNickname(ctx, Amount)
            OldAmount = Amount
        else:
            logger.info("Amount has not changed")
            await Holders.checkNickname(ctx, Amount)

def setup(bot: commands.Bot):
    bot.add_cog(Holders(bot))
